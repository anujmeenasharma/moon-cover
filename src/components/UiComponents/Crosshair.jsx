...
const Crosshair = ({ color = 'white', containerRef = null }) => {
  ...
  useEffect(() => {
    ...
    // Hide on scroll and show again shortly after scrolling stops
    let scrollTimeout;
    const handleScroll = () => {
      gsap.to(cursorRef.current, { opacity: 0, duration: 0.2, overwrite: 'auto' });
      clearTimeout(scrollTimeout);
      scrollTimeout = setTimeout(() => {
        gsap.to(cursorRef.current, { opacity: 0.35, duration: 0.3, overwrite: 'auto' });
      }, 150);
    };
